        ...
        <nav className="sidebar-nav">
          <ul>
            ...
            <li className={`nav-item${activeItem === 'Tickets' ? ' active' : ''}`}
              ...
              <Link to="/User-portal/newticket" className="nav-link">
                <span className="nav-icon"><Ticket size={18} /></span>
                {!collapsed && <span className="nav-text">Tickets</span>}
              </Link>
            </li>
            {/* RESENT TICKETs */}
            <li className={`nav-item${activeItem === 'Recent Tickets' ? ' active' : ''}`}
              onClick={() => {
                setActiveItem('Recent Tickets');
                setShowMonitoringSubmenu(false);
                closeMobileMenu();
              }}>
              <Link to="/User-portal/newticket" className="nav-link">
                <span className="nav-icon"><GeoAlt size={18} /></span>
                {!collapsed && <span className="nav-text">Recent Tickets</span>}
              </Link>
            </li>
            {/* Ticket status */}
            <li className={`nav-item${activeItem === 'Ticket Status' ? ' active' : ''}`}
              onClick={() => {
                setActiveItem('Ticket Status');
                setShowMonitoringSubmenu(false);
                closeMobileMenu();
              }}>
              <Link to="/User-portal/newticket" className="nav-link">
                <span className="nav-icon"><GeoAlt size={18} /></span>
                {!collapsed && <span className="nav-text">Ticket Status</span>}
              </Link>
            </li>
